...
Set
    j          "tests in batch"        /j1*j2/

Scalars
    mfdr_alpha "mFDR control level"    / 0.05 /
    Wd         "dollar wealth"         / 998 /
    Wa         "alpha wealth"          / 9.339784897588759e-06 /;

Parameters
    muA(j)     "alternative mean"      /set.j 2.0/
    mu0(j)     "null mean"             /set.j 0.0/
    sigma(j)   "std dev of data"       /set.j 1.0/
    q0(j)      "prior of the null"     / j1=0.3022602314569969, j2=0.09552243760161125 /
...
alpha_next(j)$(not sameas(j,'j2')).. W(j+1)       =e= W(j) - phi(j) + (q0(j)*alpha(j) + (1-q0(j))*rho(j)) * psi(j);
